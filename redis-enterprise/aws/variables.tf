...
variable "payment_method_id" {
  type        = string
  default     = null
  description = "Payment method ID (required when payment_method is credit-card). Use data.rediscloud_payment_method if needed."
}

variable "cloud_account_id" {
  type        = number
  default     = null
  description = "Redis Cloud account ID for this cloud. Default = 1 (Redis Labs internal account)"
}
...
variable "preferred_availability_zones" {
  type        = list(string)
  default     = []
  description = "Preferred AZs (e.g. use1-az1, use1-az2, use1-az5). Required by provider when multiple_availability_zones is true."
}
