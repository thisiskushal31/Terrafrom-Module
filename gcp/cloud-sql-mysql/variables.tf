...
variable "ip_configuration" {
  description = "The ip_configuration settings subblock. Ignored when proxy_iam_only = true."
  type = object({
...
variable "proxy_iam_only" {
  description = "When true: allow connections only via Cloud SQL Proxy with IAM (user_host = cloudsqlproxy~%, require_ssl = true, no authorized_networks). Set vpc_network, allocated_ip_range, assign_public_ip for networking."
  type        = bool
  default     = false
}

variable "vpc_network" {
  description = "VPC network for private IP when proxy_iam_only = true. Self link or name."
  type        = string
  default     = null
}

variable "allocated_ip_range" {
  description = "Existing allocated IP range name for Private IP when proxy_iam_only = true."
  type        = string
  default     = null
}

variable "assign_public_ip" {
  description = "When proxy_iam_only = true: set to true to assign a public IP (access still only via Cloud SQL Proxy + IAM)."
  type        = bool
  default     = false
}
