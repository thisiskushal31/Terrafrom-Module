...
locals {
  ...
  # When proxy_iam_only = true: all access via Cloud SQL Proxy + IAM (require_ssl, no authorized_networks, user_host = cloudsqlproxy~%)
  proxy_iam_ip_configuration = {
  ...
  ip_configuration_enabled = var.proxy_iam_only || length(keys(var.ip_configuration)) > 0

  ip_configurations = {
    enabled  = var.proxy_iam_only ? local.proxy_iam_ip_configuration : var.ip_configuration
    disabled = {}
  }

  # When proxy_iam_only: users connect only via Cloud SQL Proxy (IAM); user_host forced to cloudsqlproxy~%
  effective_user_host = var.proxy_iam_only ? "cloudsqlproxy~%" : var.user_host
...
resource "google_sql_user" "default" {
  ...
  project  = var.project_id
  instance = google_sql_database_instance.default.name
  host     = local.effective_user_host
...
resource "google_sql_user" "additional_users" {
  ...
  host     = each.value.host == null ? local.effective_user_host : each.value.host
  instance = google_sql_database_instance.default.name
